...
SAML.prototype.validateResponse = function (samlResponse, callback) {
  ...
  parser.parseString(xml, function (err, doc) {
    // Verify signature. This is optional as per OASIS specs.
    ...
    if (response) {
      //test if the response has encrypted assertions
      var encAssertion = self.getElement(response, 'EncryptedAssertion');
      var assertion = self.getElement(response, 'Assertion');
        if (!assertion && ! encAssertion) {
            return callback(new Error('Missing SAML assertion'), null, false);
        }

      if(encAssertion){
          //decrypt the assertion block to extract the real assertions
          if (!self.options.privateCert){
              return callback(new Error('SAML Response contains Encrypted Assertions and no privateKey was specified in options.key'), null, false);
          }
          xmlenc.decrypt(xml,{key : self.options.privateCert}, function(err, result) {
             if(err) return callback(err, null, false);
              //parse the returned decrypted fragment. remove extra non ascii characters at the end of the xml resulted from incompatible trimming
              result = result.substring(0, result.lastIndexOf(':Assertion>') + 11) ;
              parser.parseString(result, function (err, doc) {
              assertion = self.getElement(doc, 'Assertion');

              self.parseProfile (assertion, self, function(err, profile){
          ...
          self.parseProfile (assertion[0], self, function(err, profile){
              if(err) callback(err, null, false);
              callback(null, profile, false);
          })
...
SAML.prototype.parseProfile = function(assertion, parser, callback){
    profile = {};
    var self = parser;
    ...
    if (profile.EmailAddress){
        profile.email = profile.mail = profile.EmailAddress;
    }
