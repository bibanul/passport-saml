      ...
      if(encAssertion){
          ...
          xmlenc.decrypt(xml,{key : self.options.privateCert}, function(err, result) {
             if(err) return callback(err, null, false);
              //parse the returned decrypted fragment. don't force strict validation since due to different padding, garbage may exist after the closing xml tag
            parser.parseString(result, {strict:false}, function (err, doc) {
